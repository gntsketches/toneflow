...
class keypress.Listener
    ...
    _key_down: (key, e) ->
        ...
        # We might have modifier keys down when coming back to this window and they might not be in _keys_down, so
        # we're doing a check to make sure we put it back in. This only works for explicit modifier keys.
